...
<body bgcolor=#25282c>
	...
	<div id="wrap">
		<div id="code" name="code">йопта бенч1(бд)
	дтнач = ТекущаяДата()
		
	ОбработатьСерв = йопта (соед)
		база = соед.Данные()
		запр = соед.Получить()
		малява("Сервер получил соединение:",соед)
		малява("Получен запрос:",запр)
	...
	отвечаю ПрошлоВремениС(дтнач)
стопэйопта

йопта бенч2(бд)
	дтнач = ТекущаяДата()

	ОбработатьHTTP = йопта (вых,вх)
		база = вх.Данные()
		запр = вх.Сообщение()
...
малява("Все завершилось просто идеально: TCP за", бенч1(база), ", HTTP за", бенч2(база), "!")

// транзакция чтения - таких может быть сколько угодно, в разных горутинах
тран = база.НачатьТранзакцию(нечотко)
хапнуть
	малява(тран.Таблица("testing").ПолучитьВсе())
